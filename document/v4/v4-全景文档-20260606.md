...
> 覆盖版本：**v2.0 ~ v2.6 + v3.0 ~ v3.6 + v4.0 + v4.1 + v4.2 + v4.3**（v1 → v1.3 → v2.0 → ... → v2.6 → v3.0 后端模块重构 → v3.1 短期记忆三级读穿写穿 → v3.2 Worker 心跳与 Embedding 重试 → v3.3 多格式文档解析 + OCR 文本清洗 → v3.4 RAG 检索重排序 → v3.5 知识加工精切 → v3.6 查询理解与检索闭环 → v4.0 前端视觉重设计 → v4.1 知识卡片系统 → v4.2 知识卡片闭环与切片可视化 → **v4.3 Antigravity 风格动态粒子背景 & 前端极简重设计（Three.js WebGL + 超椭圆 SDF 形态渐变 + 三色流动 + Modern Minimalist Monochrome 配色体系 + Canvas 几何鱼 Logo + Space Grotesk 品牌字 + 全组件黑白化）**）
...
- **[v4.3-Antigravity风格动态粒子背景与前端极简重设计](v4.3-Antigravity风格动态粒子背景与前端极简重设计-20260608.md) — v4.3 增量记录（Three.js WebGL 粒子 + 超椭圆 SDF + 三色流动 + Monochrome 配色 + Canvas Logo + 五阶段全组件黑白化）**
...
**前端**：Vue 3 + Three.js WebGL Antigravity 粒子背景 + Modern Minimalist Monochrome 配色 + Canvas 几何鱼 Logo + 明暗双主题（详见 [v4.3 文档](v4.3-Antigravity风格动态粒子背景与前端极简重设计-20260608.md)）。
...
flowchart TB
    subgraph Browser["浏览器"]
        ...
        Particles["Three.js WebGL<br/>Antigravity 粒子背景<br/>超椭圆SDF+三色流动 [v4.3]"]
        SPA --> LS
        SPA --> Particles
    end

    subgraph FE_Runtime["前端运行时 [v4.3 极简重设计]"]
        Router["vue-router 4<br/>beforeEach 鉴权"]
        Pinia["Pinia auth / chat"]
        Theme["useTheme<br/>明暗切换（默认深色）"]
        Header["AppHeader<br/>实底导航栏 + Canvas Logo"]
        Drawer["DrawerSidebar<br/>实底抽屉式会话列表"]
        SSE["SSE 流式消费<br/>Fetch + AbortController"]
        Glass["Monochrome 配色体系<br/>95% 黑白 + 灰调语义色"]
        Router --> Pinia
        Pinia --> Theme & SSE
        Header --> Drawer
        Theme --> Particles
...
| **粒子背景** | **Three.js + Poisson Disk Sampling** | **v4.3：Antigravity 风格 WebGL 粒子，自定义 ShaderMaterial + 超椭圆 SDF 形态渐变 + 三色流动 + 鼠标引力连线** |
| SSE 消费 | Fetch API + ReadableStream | `AbortController` 取消生成 |
| Markdown | marked + highlight.js | 代码高亮, 内联代码着色 |
| **明暗主题** | **`useTheme` composable** | **v4.3：默认深色，CSS 变量约 40 项（黑白 Token）+ Element Plus 全组件覆盖 + 灰调语义色** |
| **CSS 体系** | **CSS 变量约 40 项** | **v4.3：95% 黑白 + 5% 灰调语义色，4 级明度梯度，去掉毛玻璃/渐变/发光** |
| **品牌标识** | **Canvas 2D + Space Grotesk** | **v4.3：Canvas 几何鱼 Logo + Space Grotesk 无衬线品牌字体** |
...
```
frontend/
├── package.json                  Vue 3 / Vite 6 / vue-router 4 / Element Plus / Pinia / three / poisson-disk-sampling
├── pnpm-lock.yaml
...
│
└── src/
    ├── main.ts                   主题初始化（默认深色）+ createApp [v4.3 去掉 tsparticles 注册]
    ├── App.vue                   <MeshBackground> + <RouterView> + onErrorCaptured 错误边界
    ├── style.css                 :root 黑白 Token（~40项）+ [data-theme='light'] 亮色覆盖 + Element Plus 灰调适配 [v4.3]
    │
    ├── composables/
    ...
    │   ├── auth.ts
    │   └── chat.ts
    │   └── poisson-disk-sampling.d.ts  poisson-disk-sampling npm 包类型声明 [v4.3]
    │
    ├── utils/time.ts             相对时间格式化
    ├── utils/fishLogo.ts         Canvas 几何鱼 Logo 绘制函数 [v4.3]
    │
    ├── views/
    ...
    │
    └── components/
        ├── MeshBackground.vue     Three.js WebGL Antigravity 粒子背景 + 超椭圆 SDF + 三色流动 [v4.3]
        ├── AppHeader.vue          实底导航栏 + Canvas Logo + Space Grotesk 品牌 [v4.3]
        ├── DrawerSidebar.vue      实底抽屉侧栏（300px + 遮罩 + 滑入动画）[v4.3]
        ├── MessageList.vue        消息流 + 粘底滚动 + 渐进式渲染 + 灰白建议卡片 [v4.3]
        ├── MessageBubble.vue      用户深灰实底 / 助手中灰实底 / 工具灰底 + Markdown + 工具卡片 [v4.3]
        ├── ChatInput.vue          实底输入框 + 白底黑字圆形发送按钮 [v4.3]
...
| 故障点 | 降级策略 | 用户体验 | 实现位置 |
|--------|----------|----------|----------|
| Redis 不可用 | 限流 fail-open，会话锁跳过 | 放行但无保护 | `RateLimitService` |
| L1/L2 记忆全 miss | 从 L3 全量加载，首次可能触发 compress（1-5s） | 冷会话首条消息略慢 | `ShortTermMemoryService` |
| RAG 检索超时/失败 | 跳过检索增强，直接用 Agent 上下文 | 回复质量降低但可用 | `RagRecall` try-catch |
| Rerank API 失败 | 降级为 RRF 原始排序 | Top-N 精度降低 | `DashScopeRagReranker` |
| LLM 查询分解失败 | 降级为 Identity（原查询不扩展） | 召回率降低 | `RagQueryExpandConfiguration` |
| Embedding 服务不可用 | 长期事实写入跳过，知识卡片 ES 写入失败 | 不影响对话，卡片暂不入检索 | `LongTermMemoryStore` / `KnowledgeCardEsSyncService` |
| Python Worker 宕机 | 孤儿补偿 30s 后标记 FAILED | 用户可重试上传 | `OrphanTaskCompensationService` |
| 模型路由失败（如 Ollama 未启动） | 启动时校验 + 清晰错误日志 | 应用无法启动，而非运行时异常 | `FishChatModelConfiguration` |
...
flowchart LR
    subgraph Creation["创建"]
        Chat["用户对话"] -->|LLM 分析| Extract["AI 提取<br/>prompt 注入分组树+关键词"]
        Manual["手动创建"] --> Direct["直接 confirmed"]
    end

    subgraph Enrich["丰富"]
        Extract --> Preview["即时预览确认"]
        Preview -->|确认| Confirmed["confirmed 卡片"]
        Preview -->|拒绝| Rejected["rejected（丢弃）"]
        Confirmed --> KW["关键词归一化入库<br/>normalized_name 去重"]
        Confirmed --> Group["分组树同步<br/>card_group 归一化"]
        Confirmed --> ESSync["ES 双写<br/>embedding + 全文索引"]
    end

    subgraph Discovery["关联"]
        KW --> Signal["多信号关联发现<br/>Jaccard 0.35 + 分组 0.20<br/>+ 祖先 0.20 + embedding 0.25"]
        Group --> Signal
        Signal -->|综合 > 0.45| Suggest["建议 related_to"]
    end

    subgraph Recall["召回"]
        ESSync -->|confirmed 卡片| RAG["四路 RAG recall<br/>UserKnowledgeCardSearcher"]
        RAG -->|text + vector 双路| Fusion["RRF 融合 → Rerank 精排"]
        Fusion -->|Top-8| Inject["注入 Agent 上下文"]
    end

    subgraph Review["复习"]
        Confirmed --> Flip["翻转卡片复习<br/>忘了/模糊/熟悉"]
    end

    Creation --> Enrich --> Discovery --> Recall
```

**为什么 confirmed 才入库 ES？**——pending 卡片质量未验证，可能包含 LLM 幻觉。confirmed 后经人工审核，质量可靠，才值得参与 RAG 检索影响对话质量。

**为什么关联发现用多信号而非纯 embedding？**——embedding 只捕获语义相似度，无法表达"同一分组""共享关键词祖先"等结构关系。例如"二叉树"和"红黑树"语义距离较远但同属"数据结构→树"分组，多信号可以捕获这种关联。
...
`AgentToolProvider` SPI 接口 + `ToolRegistry` 自动发现，内置 5 + 外部 5 共 10 个工具按 `@ConditionalOnProperty` 懒装配。三重防死循环（recursionLimit + ModelCallLimitHook 优雅终止 + AgentStatus 状态机）。

| 工具 | 类型 | 触发场景 | 外部依赖 |
|------|------|----------|----------|
| DateTime | 内置 | "现在几点""今天日期" | 无 |
| Calculator | 内置 | 数学表达式计算 | 无 |
| FileRead / FileWrite | 内置 | 读写用户文件 | RustFS |
| WebFetch | 外部 (Tavily) | "帮我查一下..." | Tavily API |
| AmapWeather / AmapGeo | 外部 (高德) | "今天天气""附近有什么" | 高德 API |
| BochaSearch | 外部 (Bocha) | 通用搜索 | Bocha API |
| Mail | 外部 (SMTP) | "发邮件给..." | SMTP 服务器 |

为什么用 SPI 而非硬编码？——新增工具只需实现 `AgentToolProvider` 接口 + 加 `@Component`，`ToolRegistry` 自动发现并注册，零配置改动。外部工具通过 `@ConditionalOnProperty` 按需加载，无 API Key 时不报错直接跳过。

**2. 四索引 RAG 检索引擎 + RRF 融合 + Cross-Encoder 精排（v3.4, v4.2）**
四索引完全隔离（`fish-user-memory` / `fish-user-knowledge` / `fish-knowledge-card` / `fish-public-knowledge`），检索时用 `userId` 过滤，**天然多租户隔离，公共知识零权限开销**。每个子查询 8 路并发召回（4 索引 × text+vector）→ **RRF 分数融合** → **DashScope `qwen3-rerank` Cross-Encoder 精排**（50→8）。

为什么不用单一混合索引？——四索引隔离后，每路检索可独立调优 BM25 字段和 kNN 参数，不需要在同一个索引里处理"对话事实"和"文档切片"完全不同的字段结构。为什么不用线性加权融合？——BM25 分数范围 [0, ∞)，cosine [-1, 1]，两者不可直接比较；**RRF 只依赖排名，对原始分布不敏感**，鲁棒性远超加权。为什么需要精排？——RRF 融合后候选 50 条注入 LLM 会严重分心，**Cross-Encoder 精排到 Top-8 大幅降低噪音**，精排失败则降级为 RRF 原始排序。
...
**5.5. 短期记忆三级读穿写穿（v3.1）**
`ShortTermMemoryService` 协调 L1(Redis) → L2(RustFS/文件快照) → L3(全量正文)。热路径只读 Redis（<1ms），~90% 活跃会话在 TTL 内命中 L1。

为什么分三层而非单层 Redis？——单层 TTL 过期后记忆永久丢失，冷会话恢复需从对象存储全量加载。三层架构保证：L1 命中时零额外 IO，L1 过期后 L2 快照兜底（~50ms），L2 也过期才走 L3 全量（仅首次可能触发 LLM 压缩，1-5s）。代价是 L2 异步刷新与 L1 写入之间有极低概率竞态，但实际影响仅是快照略旧，下次刷新即修正。

**5.6. 查询理解与检索闭环（v3.6）**
LLM 语义查询分解 + HyDE（默认关闭）+ 长期事实 embedding 余弦查重（>0.92 阈值）+ RAG 质量追踪异步写 ES。

为什么用 LLM 分解而非 Token 级分词？——"Java 和 Python 的区别"用分词会拆成"Java""Python"两个孤立查询，丢失"对比"语义。LLM 分解为"Java 语言特点""Python 语言特点""两者对比分析"三个子查询，召回率显著提升。HyDE 为什么默认关闭？——假设性文档嵌入对事实性查询有帮助，但对**对话型查询会引入偏差**（模型假设的答案可能误导向量检索），保守策略下关闭，仅对文档检索场景可选开启。
...
**10. Antigravity 风格 Three.js WebGL 动态粒子背景（v4.3）**
`MeshBackground.vue` 使用 **Three.js + Poisson Disk Sampling + 自定义 ShaderMaterial**，`position: fixed` z-index: 0 全局渲染。参考 Google Antigravity 源码，实现 **超椭圆 SDF 形态渐变**（`pow(|x|, n) + pow(|y|, n)`，n=2 圆形 ↔ n=1 菱形，由鼠标距离 `smoothstep` 驱动丝滑变形）、**三色流动**（暗色 `#7189ff`/`#f84242`/`#ffcf03`，亮色 `#2c64ed`/`#f84242`/`#ffcf03`，三正弦波 120° 间隔叠加归一化）、**鼠标引力连线**（Raycaster 世界坐标 + 粒子间连线 0.18 + 鼠标引力线 0.5）、**呼吸 + 漂浮 + 旋转加速**。`PerspectiveCamera(fov=40, z=3.1)`，粒子按可视范围拉伸铺满全屏。NormalBlending 避免过曝。主题切换同步 scene.background + 三色 + 连线灰度。

**11. Modern Minimalist Monochrome 配色体系（v4.3）**
去掉全部毛玻璃、渐变按钮、发光阴影。改为 **95% 黑白 + 5% 灰调语义色**，4 级明度梯度（`#0a0a0a` / `#060606` / `#141414` / `#1c1c1c`）。所有组件硬编码 RGBA 抽取为 CSS 变量（`--bg-elevated` / `--bg-surface` / `--border` / `--border-bright` / `--status-ok` 等），切换主题时所有组件背景自适应。按钮体系 5 种（Primary 白底黑字 / Secondary 灰边框 / Ghost 透明 / Danger 灰红 / Success 灰绿）。

**12. 抽屉式侧栏 + 居中单栏聊天（v4.0 → v4.3 实底化）**
`DrawerSidebar` 通过 `Teleport to="body"` 渲染在 body 层，`useDrawer` composable 全局共享开关状态。从左滑入 300px + 实底深灰背景 `#1c1c1c`（去掉毛玻璃）。聊天区无固定侧栏，消息居中 max 800px，最大化粒子背景可视面积。`AppHeader` 实底背景 + Canvas 几何鱼 Logo + Space Grotesk 品牌字。

**13. 明暗双主题 + CSS 变量化（v4.0 → v4.3 黑白 Token）**
`useTheme` composable 管理 `data-theme` 属性 + Element Plus `.dark` class + localStorage 持久化，默认深色。`:root` 定义约 40 项黑白 Token，`[data-theme='light']` 完整覆盖亮色值（背景 `#fafafa` / 边框 `rgba(0,0,0,0.08)` / 语义色反转为深色版）。Element Plus 全组件灰调覆盖（表格、分页、按钮、输入框、卡片、Tabs、MessageBox）。

**14. Canvas 几何鱼 Logo + Space Grotesk 品牌标识（v4.3）**
`utils/fishLogo.ts` 纯 Canvas 2D 绘制几何化鱼形线描（贝塞尔曲线鱼身 + 实心圆眼睛 + V 形尾鳍 + 渐隐拖尾），替代 emoji 🐟。品牌文字使用 Google Fonts CDN 引入的 Space Grotesk 无衬线字体（font-weight 600, letter-spacing -0.02em）。Logo 尺寸按位置分级（Header 24px / 登录页 48px / 欢迎 56px / 空态 40px），极慢微摆动画 ±2deg/8s。

---

### 关键 Trade-off 决策表

> 面试高频问题速查："你为什么这么做？代价是什么？考虑过什么替代方案？"

| # | 决策点 | 选择 | 放弃的替代方案 | 理由 | 代价 |
|---|--------|------|---------------|------|------|
| 1 | 短期记忆分层 | L1 Redis → L2 快照 → L3 全量 | 单层 Redis + TTL | 热路径 <1ms，~90% 命中 L1；冷会话有回填路径 | L2 快照可能略旧，极低概率竞态 |
| 2 | RAG 分数融合 | RRF（倒数排名） | 线性加权 / 归一化分数 | RRF 对原始分数分布不敏感，BM25 和 cosine 可直接融合 | 丢失绝对分数信息，无法区分"都很相关"和"都不太相关" |
| 3 | 查询扩展 | LLM 语义分解 | Token 级分词扩展 | 理解意图而非拆词，"对比"类查询不会被拆散 | 额外 LLM 调用（~200ms），不可用时降级为 Identity |
| 4 | Rerank 精排 | DashScope Cross-Encoder | 本地 Cross-Encoder | SaaS 免部署，中文优化，与 LLM 共用 API Key | 200-400ms 额外延迟，外部依赖 |
| 5 | 知识卡片关联 | 多信号加权融合 | 纯 embedding 相似度 | 结构信号（分组/关键词）捕获语义相似度无法表达的关系 | O(n²) 计算复杂度，需前端手动触发 |
| 6 | Worker 终态写入 | CAS 乐观锁 | 分布式锁（Redis SET NX） | 无锁竞争，正常路径零开销 | 并发修改时需 ES 兜底清理 |
| 7 | Agent 防死循环 | 三重防护 | 单一 recursionLimit | ModelCallLimitHook 优雅终止 + AgentStatus 状态机可外部干预 | ReactAgent 内部循环对用户是黑盒 |
| 8 | 知识索引分离 | 四索引完全独立 | 单一混合索引 | 每路可独立调优 BM25/kNN 参数，天然多租户隔离 | 四份索引的存储和写入开销 |
| 9 | HyDE 假设文档 | 默认关闭 | 默认开启 | 对话型查询 HyDE 引入偏差，事实性查询才受益 | 文档检索场景需手动开启 |
| 10 | 长期事实查重 | embedding 余弦 > 0.92 | 不查重直接写入 | 避免同一事实反复入库导致检索噪音 | 每条事实需额外一次 embedding 计算 |
...
| 职责 | 路径 | 说明 |
|------|------|------|
| **粒子背景** | `components/MeshBackground.vue` | **[v4.3 替换]** Three.js WebGL Antigravity 粒子 + 超椭圆 SDF + 三色流动 + 鼠标引力线 |
| **Canvas Logo** | `utils/fishLogo.ts` | **[v4.3 新增]** Canvas 2D 几何鱼绘制函数 |
| **Poisson 类型** | `types/poisson-disk-sampling.d.ts` | **[v4.3 新增]** TypeScript 类型声明 |
| **顶部导航栏** | `components/AppHeader.vue` | **[v4.0 新增, v4.3 重构]** 实底导航 + Canvas Logo + Space Grotesk |
| **抽屉侧栏** | `components/DrawerSidebar.vue` | **[v4.0 新增, v4.3 重构]** 实底抽屉 300px，会话列表+上传区 |
| **抽屉状态** | `composables/useDrawer.ts` | **[v4.0 新增]** 全局抽屉开关状态 |
| 主题管理 | `composables/useTheme.ts` | **[v4.0 恢复]** 明暗切换，默认深色 |
| 全局样式 | `style.css` | **[v4.3 重写]** 黑白 Token ~40 项 + 亮色覆盖 + 灰调语义色 + EP 灰调适配 |
| 根组件 | `App.vue` | **[v4.3 修改]** 挂载 MeshBackground |
| 聊天页 | `views/ChatView.vue` | **[v4.0 重写]** AppHeader + DrawerSidebar + 居中单栏 |
| 登录页 | `views/LoginView.vue` | **[v4.3 重构]** 实底卡片 + Canvas Logo + 白底黑字按钮 |
| 知识库页 | `views/KnowledgeView.vue` | **[v4.3 重构]** 实底头部 + 实底容器 |
| 消息气泡 | `components/MessageBubble.vue` | **[v4.3 重构]** 用户深灰 + 助手中灰实底 |
| 输入框 | `components/ChatInput.vue` | **[v4.3 重构]** 实底 + 白底黑字发送按钮 |
| 消息列表 | `components/MessageList.vue` | **[v4.3 重构]** 灰白建议卡片 + Canvas Logo |
| 入口 | `main.ts` | **[v4.3 修改]** 去掉 tsparticles 注册 |
...
- ~~**粒子背景升级**：从 tsParticles 替换为 Three.js WebGL Antigravity 风格粒子~~ ✅ v4.3 已完成
- ~~**前端极简重设计**：去掉毛玻璃/渐变/发光，改为黑白 Monochrome 配色~~ ✅ v4.3 已完成
- ~~**Canvas Logo + Space Grotesk**：几何鱼线描 Logo + 无衬线品牌字体~~ ✅ v4.3 已完成
- **粒子配置外部化**：将 Three.js 粒子数量/速度/颜色移入 CSS 变量或环境变量，方便部署时调优
- **移动端适配**：抽屉侧栏在小屏上宽度自适应，Three.js 粒子数量根据屏幕尺寸降低
...
_文档版本：**v4 全景（v2.0 ~ v2.6 + v3.0 ~ v3.6 + v4.0 + v4.1 + v4.2 + v4.3 合并）** · 最后更新：2026-06-08_

...
> 覆盖版本：**v2.0 ~ v2.6 + v3.0 ~ v3.6 + v4.0**（v1 → v1.3 → v2.0 → ... → v2.6 → v3.0 后端模块重构 → v3.1 短期记忆三级读穿写穿 → v3.2 Worker 心跳与 Embedding 重试 → v3.3 多格式文档解析 + OCR 文本清洗 → v3.4 RAG 检索重排序 → v3.5 知识加工精切 → v3.6 查询理解与检索闭环 → **v4.0 前端视觉重设计（tsParticles 粒子背景 + 毛玻璃拟态 + 深色蓝紫基调 + 抽屉式侧栏 + 明暗双主题）**）
> 后端 Java 源文件约 **113** 个，`frontend/src` 下 TS/Vue 约 **24** 个，Python Worker 约 **21** 个文件。
...
└── src/
    ...
    │   ├── ChatView.vue          AppHeader + DrawerSidebar + 居中消息区（max 800px）[v4.0]
...
`fish-user-memory` + `fish-user-knowledge` + `fish-public-knowledge` 三索引完全分离。每个子查询 6 路并发召回 → **RRF 分数融合** → **DashScope `qwen3-rerank` Cross-Encoder 精排**（50 条候选 → Top-8 注入）。降级链完整。
...
| DashScope Rerank | `rag/pipeline/rerank/DashScopeRagReranker.java` [v3.4] |
...
- **编码检测增强**：引入 charset-normalizer 替代硬编码候选列表
...
_文档版本：**v4 全景（v2.0 ~ v2.6 + v3.0 ~ v3.6 + v4.0 合并）** · 最后更新：2026-06-06_

...
> 覆盖版本：**v2.0 ~ v2.6 + v3.0**（v1 → v1.3 → v2.0 → ... → v2.6 → **v3.0 后端模块重构**）
...
**前端**：Vue 3 + TypeScript + Element Plus，SSE 流式呈现，暗夜模式持久化；v2.5 前端全面美化；v2.6 可靠性与体验改进；**v3.0 后端包结构从按技术层分包重构为按业务域分包，零功能变更**。
...
flowchart TB
    ...
    subgraph Storage["存储层"]
        Redis["Redis DB2<br/>Session / 短期摘要 / Stream / 限流 / 会话锁"]
...
| 缓存 | Redis (Lettuce) | DB2：Session + 短期摘要 + Stream + 限流 + 会话互斥 |
...
│   │   ├── ShortTermMemoryStore.java          SPI 接口
...
sequenceDiagram
    ...
    alt v2.3 限流拒绝
        ...
        BE->>STM: load 短期摘要窗口
        alt Redis 有摘要 (history ≥ 30)
            STM-->>BE: summary + recentMessages
        else Redis 无摘要 (history < 30)
            BE->>BE: 从文件历史截取最近 20 条
        end
        BE->>RAG: 三路并发检索
        RAG->>ES: 并发搜索
        ES-->>RAG: 命中列表
        ...
        BE->>LTM: 异步抽取长期事实
        alt history ≥ 30 (触发压缩)
            BE->>STM: 异步 LLM 压缩 → 写入 Redis
        end
    end
```

### 8.2 短期记忆懒压缩流程（v3.0 文档梳理）
...
_文档版本：**v3 全景（v2.0 ~ v2.6 + v3.0 合并）** · 最后更新：2026-05-12_

...
> 覆盖版本：**v2.0 ~ v2.6 + v3.0 + v3.1 + v3.2 + v3.3 + v3.4**（v1 → v1.3 → v2.0 → ... → v2.6 → v3.0 后端模块重构 → v3.1 短期记忆三级读穿写穿 → v3.2 Worker 心跳与 Embedding 重试 → v3.3 多格式文档解析 + OCR 文本清洗 → **v3.4 RAG 检索重排序（RRF 融合 + DashScope Rerank）**）
> 后端 Java 源文件约 **113** 个，`frontend/src` 下 TS/Vue 约 **21** 个，Python Worker 约 **19** 个文件。
...
**核心能力链**：登录鉴权 → **`/api/chat/**` Redis 令牌桶 + SSE 并发限流** → **同 `sessionId` Redis 互斥（v2.4，防双击竞态）** → 多轮对话（SSE 流式）→ **三级短期记忆（v3.1：L1 Redis → L2 对象存储快照 → L3 全量正文，热路径只读 Redis）** / 长期事实抽取（v2.4）→ **三路 ES RAG 检索 → RRF 分数融合 + DashScope Rerank 精排（v3.4）** → 工具调用。
...
**前端**：Vue 3 + TypeScript + Element Plus，SSE 流式呈现，暗夜模式持久化；v2.5 前端全面美化；v2.6 可靠性与体验改进；v3.0 后端包结构从按技术层分包重构为按业务域分包；v3.1 短期记忆三级读穿写穿（L1/L2/L3），热路径只读 Redis；v3.2 Worker 心跳 + Embedding 重试，长任务不误杀、瞬时错误自动重试；v3.3 多格式文档解析（txt/md/docx/html/xlsx/pptx）+ OCR 文本统一清洗管道；**v3.4 RAG 检索重排序（RRF 分数融合 + DashScope Cross-Encoder 精排）**。
...
flowchart TB
    ...
    subgraph BE["Spring Boot 后端 (JDK 21 虚拟线程)"]
        ...
        RAG["RagRecall<br/>三路并发检索<br/>+ RRF融合 + Rerank[v3.4]"]
...
│   │   ├── recall/
│   │   │   ├── RagRecall.java                 编排 + RRF融合 + Rerank精排 + 渲染注入块
│   │   │   ├── RagRecallConfiguration.java    三路 Searcher + Reranker Bean 装配
...
│   │       └── DashScopeRagReranker.java      DashScope gte-rerank 实现 + 降级 [v3.4]
│   ├── document/                               ES 文档模型
...
`fish-user-memory` + `fish-user-knowledge` + `fish-public-knowledge` 三索引完全分离。每个子查询 6 路并发召回 → **RRF（Reciprocal Rank Fusion）分数融合**统一 BM25 与 cosine 不可比的排名 → **DashScope `gte-rerank` Cross-Encoder 精排**（50 条候选 → Top-8 注入）。Reranker 失败自动降级到融合结果，`fusion.enabled=false` 时回退旧 `mergeByMaxScore`。
...
初检索三路双路并发 → `RagScoreFusion` RRF 排名制融合（统一 BM25/cosine 不可比分数）→ `DashScopeRagReranker` Cross-Engine 精排（`gte-rerank` API，50 条候选 → Top-8 注入）；RestClient 构造一次复用；三级降级（disabled/无Key/异常）内聚在实现中，编排层无 if/else；接口+实现分离，可替换为本地模型；`per-subquery-size` 5→10、`knn-num-candidates` 80→120 扩大候选池。
...
| `FISH_RAG_RERANK_MODEL` | 否 | `gte-rerank` | v3.4 Rerank 模型名 |
...
- **长期事实去重**：写入前按 embedding 余弦相似度幂等过滤（v3.6 候选）
- **LLM 语义查询扩展**：替换当前词级拆解为 LLM 驱动的查询分解（v3.6 候选）
- **RAG 检索质量追踪**：记录每次 RAG 全链路指标（query / 命中数 / rerank 分数 / 延迟）写入 ES，数据驱动调优（v3.6 候选）
- **结构化分块策略**：按文档类型差异化分块（Title+Text 合并、Table 整表保留、句子边界对齐）（v3.5 候选）
- **PDF 混合提取策略**：先尝试直接文本提取 → 检测乱码 → 回退 OCR；文字型 PDF 可获完美文本并大幅提速（v3.5 候选）
- **ES 元数据丰富**：写入时携带 `doc_name` / `file_type` / `token_count`（v3.5 候选）
...
_文档版本：**v3 全景（v2.0 ~ v2.6 + v3.0 + v3.1 + v3.2 + v3.3 + v3.4 合并）** · 最后更新：2026-06-05_

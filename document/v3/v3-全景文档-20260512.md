...
> 覆盖版本：**v2.0 ~ v2.6 + v3.0 + v3.1 + v3.2 + v3.3 + v3.4 + v3.5 + v3.6**（v1 → v1.3 → v2.0 → ... → v2.6 → v3.0 后端模块重构 → v3.1 短期记忆三级读穿写穿 → v3.2 Worker 心跳与 Embedding 重试 → v3.3 多格式文档解析 + OCR 文本清洗 → v3.4 RAG 检索重排序 → v3.5 知识加工精切 → **v3.6 查询理解与检索闭环（LLM 查询分解 + 事实查重 + 质量追踪）**）
> 后端 Java 源文件约 **113** 个，`frontend/src` 下 TS/Vue 约 **21** 个，Python Worker 约 **21** 个文件。
...
- **[v3.4-检索重排序](v3.4-检索重排序-20260605.md) — v3.4 增量记录（RRF 融合 + DashScope Rerank 精排）**
- **[v3.5-知识加工精切](v3.5-知识加工精切-20260605.md) — v3.5 增量记录（结构化分块 + PDF 混合提取 + ES 元数据）**
- **[v3.6-查询理解与检索闭环](v3.6-查询理解与检索闭环-20260606.md) — v3.6 增量记录（LLM 查询分解 + 事实查重 + 质量追踪）**
...
**核心能力链**：登录鉴权 → **`/api/chat/**` Redis 令牌桶 + SSE 并发限流** → **同 `sessionId` Redis 互斥（v2.4，防双击竞态）** → 多轮对话（SSE 流式）→ **三级短期记忆（v3.1：L1 Redis → L2 对象存储快照 → L3 全量正文，热路径只读 Redis）** / 长期事实抽取（v2.4，**v3.6：写入前 embedding 余弦查重**）→ **三路 ES RAG 检索 → LLM 语义查询分解（v3.6）→ RRF 分数融合 + DashScope Rerank 精排（v3.4）** → 工具调用。**RAG 全链路质量追踪异步写 ES（v3.6）**。

**对话模型路由**：支持 **DashScope**（阿里云通义）/ **Ollama**（本地）/ **DeepSeek**（OpenAI-compatible）三路可选，嵌入独立配置，切换只需修改一个环境变量，上层业务零感知。

**知识库闭环**：前端上传（直传/分片）→ Java 写入 RustFS + MySQL + Redis Stream → Python Worker 异步消费 → **多格式解析（v3.3：PDF OCR / txt / md / docx / html / xlsx / pptx）** → **文本清洗管道（v3.3：NFC / 控制字符 / OCR 替换 / 标点合并 / 空白标准化）** → **结构化分块（v3.5：Title+Text 合并 / Table 整表保留 / 句子边界对齐，可回退 flat 滑窗）** → embedding 向量化（v3.2：429/5xx 指数退避重试）→ **ES 批量写入（v3.5：+ doc_name / file_type / token_count 元数据）** → 前端管理页列表/删除。Worker 心跳（v3.2）防长任务被孤儿补偿误杀，终态 CAS + ES 清理保证数据一致。

**前端**：Vue 3 + TypeScript + Element Plus，SSE 流式呈现，暗夜模式持久化；v2.5 前端全面美化；v2.6 可靠性与体验改进；v3.0 后端包结构从按技术层分包重构为按业务域分包；v3.1 短期记忆三级读穿写穿（L1/L2/L3），热路径只读 Redis；v3.2 Worker 心跳 + Embedding 重试，长任务不误杀、瞬时错误自动重试；v3.3 多格式文档解析（txt/md/docx/html/xlsx/pptx）+ OCR 文本统一清洗管道；v3.4 RAG 检索重排序（RRF 分数融合 + DashScope Cross-Encoder 精排）；v3.5 知识加工精切（结构化分块 + PDF 混合提取 + ES 元数据）；**v3.6 查询理解与检索闭环（LLM 语义查询分解 + 事实查重 + RAG 质量追踪）**。
...
flowchart TB
    ...
    subgraph BE["Spring Boot 后端 (JDK 21 虚拟线程)"]
        ...
        RAG["RagRecall<br/>三路并发检索<br/>+ LLM查询分解[v3.6]<br/>+ RRF融合 + Rerank[v3.4]<br/>+ 质量追踪[v3.6]"]
...
│   │   ├── recall/
│   │   │   ├── RagRecall.java                 编排 + LLM分解[v3.6] + RRF融合 + Rerank精排 + 质量埋点[v3.6] + 渲染注入块
│   │   │   ├── RagRecallConfiguration.java    三路 Searcher + Reranker + HyDE + Logger Bean 装配 [v3.6]
...
│   │   │   └── DashScopeRagReranker.java      DashScope qwen3-rerank 实现 + 降级 [v3.4]
│   │   └── tracing/
...
**2. 三索引 RAG 检索引擎 + RRF 融合 + Rerank 精排（v3.4）**
`fish-user-memory` + `fish-user-knowledge` + `fish-public-knowledge` 三索引完全分离。每个子查询 6 路并发召回 → **RRF（Reciprocal Rank Fusion）分数融合**统一 BM25 与 cosine 不可比的排名 → **DashScope `qwen3-rerank` Cross-Encoder 精排**（50 条候选 → Top-8 注入）。Reranker 失败自动降级到融合结果，`fusion.enabled=false` 时回退旧 `mergeByMaxScore`。
...
**4.5. 知识加工精切（v3.5）**
结构化分块按 `element_type` 差异化：Title+Text 合并 Section → 句子边界贪心打包（overlap 句子级回收）；Table 整表保留（≤ 1024 token）或行组切分+表头重复；PDF 文本层优先+质量检测（CID/乱码/长度）→ OCR 回退；ES 新增 `doc_name` / `file_type` / `token_count` 元数据；`FISH_WORKER_CHUNK_STRATEGY=flat` 可即时回退旧版滑窗。
...
**9.7. RAG 检索重排序：RRF 融合 + Cross-Encoder 精排（v3.4）**
初检索三路双路并发 → `RagScoreFusion` RRF 排名制融合（统一 BM25/cosine 不可比分数）→ `DashScopeRagReranker` Cross-Engine 精排（`qwen3-rerank` API，50 条候选 → Top-8 注入）；RestClient 构造一次复用；三级降级（disabled/无Key/异常）内聚在实现中，编排层无 if/else；接口+实现分离，可替换为本地模型；`per-subquery-size` 5→10、`knn-num-candidates` 80→120 扩大候选池。

**9.8. 查询理解与检索闭环（v3.6）**
查询扩展从词级 `BreakIterator` 拆解升级为 `LlmQueryDecomposer` 语义分解（`memoryChatModel` 生成 1~4 条完整检索句，超时 3s 降级单条原句）；策略三档可切换（LLM / TOKEN / IDENTITY）；可选 HyDE 假设性文档嵌入替换向量腿 embedding 文本（默认关闭）；长期事实写入前 `LongTermMemoryDeduplicator` embedding 余弦查重（ES knn 取回邻居 + 精确余弦 ≥ 0.92 判定重复跳过，查重失败不阻塞写入）；`RagQualityLogger` 每次检索异步记录 19 字段全链路指标（query/子查询/命中数/rerank 分数/HyDE 状态/延迟分解）写入 ES `fish-rag-trace`，采样率可配置，写入失败仅 WARN 不阻塞主流程。
...
| `FISH_RAG_RERANK_MODEL` | 否 | `qwen3-rerank` | v3.4 Rerank 模型名（原 `gte-rerank` 已下线） |
| `FISH_WORKER_CHUNK_STRATEGY` | 否 | `structured` | v3.5 分块策略：`structured` 结构化 / `flat` 旧版滑窗 |
| `FISH_WORKER_TABLE_MAX_TOKENS` | 否 | `1024` | v3.5 表格整表保留 token 上限 |
| `FISH_RAG_EXPAND_ENABLED` | 否 | `true` | v3.6 查询扩展开关（false → 单条原句） |
...
| 三路 RAG 召回 | `rag/pipeline/recall/RagRecall.java`（v3.6：LLM 分解 + HyDE + 质量埋点） |
| 查询扩展（LLM 分解 / 词级 / 原句） | `rag/pipeline/expand/RagQueryExpand.java` [v3.6] |
| 查询扩展装配 | `rag/pipeline/expand/RagQueryExpandConfiguration.java` [v3.6] |
| LLM 分解 Prompt | `rag/pipeline/expand/RagQueryDecomposePrompt.java` [v3.6] |
| HyDE 假设性文档嵌入 | `rag/pipeline/expand/RagHydeService.java` [v3.6] |
| RAG 质量追踪日志文档 | `rag/tracing/RagTraceDocument.java` [v3.6] |
| RAG 质量追踪写入器 | `rag/tracing/RagQualityLogger.java` [v3.6] |
| 长期事实查重 | `memory/longterm/LongTermMemoryDeduplicator.java` [v3.6] |
| RRF 分数融合 | `rag/pipeline/fusion/RagScoreFusion.java` [v3.4] |
| Rerank 精排接口 | `rag/pipeline/rerank/RagReranker.java` [v3.4] |
| DashScope Rerank 实现 | `rag/pipeline/rerank/DashScopeRagReranker.java` [v3.4] |
...
| Python 处理管线 | `python/fish_worker/processor.py`（v3.2：心跳 + CAS + ES 清理；v3.5：策略分块 + file_type） |
| Python PDF 混合提取 | `python/fish_worker/parser/pdf.py`（v3.5：文本层优先 + OCR 回退） |
...
| Python 句子边界切分 | `python/fish_worker/chunker/sentence_splitter.py` [v3.5] |
| Python 结构化分块 | `python/fish_worker/chunker/structured_chunker.py` [v3.5] |
| Python Embedding（含重试） | `python/fish_worker/chunker/embedder.py`（v3.2：HTTP 重试） |
| Python ES 写入 | `python/fish_worker/storage/elasticsearch.py`（v3.5：doc_name/file_type/token_count） |
...
- ~~**结构化分块策略**~~：**v3.5 已完成**（Title+Text 合并 / Table 整表保留 / 句子边界对齐 / 策略可切换）
- ~~**PDF 混合提取策略**~~：**v3.5 已完成**（文本层优先 → CID/乱码检测 → OCR 回退）
- ~~**ES 元数据丰富**~~：**v3.5 已完成**（doc_name / file_type / token_count）
- **编码检测增强**：引入 charset-normalizer 替代硬编码候选列表，覆盖更多编码
- **OCR 引擎升级**：PaddleOCR 替换 Tesseract，中英文准确率从 ~95% 提升至 ~99%
- ~~**长期事实去重**~~：**v3.6 已完成**（embedding 余弦查重 ≥ 0.92 跳过）
- ~~**LLM 语义查询扩展**~~：**v3.6 已完成**（LlmQueryDecomposer + HyDE + 策略三档）
- ~~**RAG 检索质量追踪**~~：**v3.6 已完成**（RagQualityLogger 异步写 ES fish-rag-trace）
...
_文档版本：**v3 全景（v2.0 ~ v2.6 + v3.0 + v3.1 + v3.2 + v3.3 + v3.4 + v3.5 + v3.6 合并）** · 最后更新：2026-06-06_

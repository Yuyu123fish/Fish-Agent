...
> 覆盖版本：**v2.0 + v2.1 + v2.2 + v2.3 + v2.4 + v2.5**（v1 → v1.3 → v2.0 → v2.1 → v2.2 → v2.3 → v2.4 → v2.5 能力线）
...
- [v2.5-前端交互与视觉优化](v2.5-前端交互与视觉优化-20260506.md) — v2.5 增量记录（新会话首发可见、disabled 按钮暗色修复、滚动条主题化、聊天页微交互优化）
...
**前端**：Vue 3 + TypeScript + Element Plus，SSE 流式呈现，暗夜模式持久化；**v2.5** 新增新会话 pending 首发可见、disabled 按钮暗色一致性、全局滚动条主题化与聊天页微交互优化。
...
| CSS 体系 | CSS 变量 17 项 | `:root` 亮色 + `[data-theme='dark']` 暗色全覆盖 |
...
│   ├── ChatController.java                      /api/chat/stream|sessions|sessions/{sid}
...
│
└── src/
    ├── main.ts                   ① 主题防闪烁初始化 → ② createApp + Pinia + router + ElementPlus
    ├── App.vue                   <RouterView />
    ├── style.css                 :root 17 项 CSS 变量 + [data-theme='dark'] 全覆盖 + Element Plus 暗色适配
    │
    ├── composables/
    ...
    │   ├── chat.ts               SSE 流式对话, AbortSignal 取消
    │   └── knowledge.ts          直传/分片上传/列表/删除/状态轮询, 11 个 API 函数
    │
    ├── store/
    │   ├── auth.ts               token + nickname localStorage 持久化
    │   └── chat.ts               sessions / activeSid / messagesBySid / streaming 流式编排
    │
    ├── types/
    │   ├── auth.ts
    │   └── chat.ts
...
**6. CSS 变量驱动的暗夜模式**
17 项 CSS 变量控制全局色彩体系，`useTheme` composable 统一同步 `data-theme`（自定义变量）+ `html.dark`（Element Plus 16 个组件内部变量）双机制。`main.ts` 在 `createApp` 之前初始化主题避免首帧闪白，`localStorage` 持久化用户偏好。
...
| Web 拦截器链注册 | `config/WebMvcConfig.java` |
...
| 对话编排（v2.4 会话锁 + SSE 释放合并） | `service/ChatService.java` |
...
| 暗夜模式 | `frontend/src/composables/useTheme.ts` |
| 主题防闪烁 | `frontend/src/main.ts` |
| CSS 变量体系 + 全局滚动条主题化（v2.5） | `frontend/src/style.css` |
| 侧栏 + 暗夜按钮 | `frontend/src/components/SessionSidebar.vue` |
| 消息气泡 + 复制 | `frontend/src/components/MessageBubble.vue` |
| 欢迎建议卡片 + 粘底滚动 + 手动平滑回底（v2.5） | `frontend/src/components/MessageList.vue` |
| 知识库管理页 | `frontend/src/views/KnowledgeView.vue` |
...
- **409 前端体验**：`chat.ts` 统一提示「会话处理中，请稍候」
...
**v2.5 已交付（自本全景文档起不再列为待办）**：新会话首条消息即时可见（`__pending__`）、plain 按钮 disabled 暗色修复、全局滚动条样式统一、MessageList/MessageBubble/SessionSidebar/ChatInput 微交互优化。

---

_文档版本：**v2 全景（v2.0 + v2.1 + v2.2 + v2.3 + v2.4 + v2.5 合并）** · 最后更新：2026-05-06_

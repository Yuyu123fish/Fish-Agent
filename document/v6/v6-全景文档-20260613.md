...
> 覆盖版本：**v2.0 ~ v2.6 + v3.0 ~ v3.6 + v4.0 ~ v4.3 + v5.0 ~ v5.6 + v6.0 + v6.1 + v6.2 + v6.3 + v6.4 + v6.5**（… → v6.2 上下文韧性（工具结果治理 A 截断 + B 摘要 + C 检索式注入）→ v6.3 长上下文适配（DeepSeek V4-Flash 1M 落地 + 治理阈值随窗口自缩放 + ActiveChatModelContext 单真相源）→ **v6.4 滚动更新（prompt-cache 排序 + 个人 KB 答案出处溯源）** → **v6.5 知识图谱链路治理（P0 静默失效 + 图谱接入召回 + 死代码清理 + per-fact trace）**）
> 后端 Java 源文件约 **198** 个（v6.0 +6、v6.1 +11、v6.2 +6、v6.3 +1、v6.4 +1、v6.5 +2：`CardGraphExpander` / `KnowledgeCardEsReconciliationService`），Python Worker 约 **22** 个文件，前端 v6.4 改动（答案出处渲染：`MessageBubble.vue` 等 4 文件）。
...
- **[v6.4-滚动更新](v6.4-长上下文Agent循环治理-20260627.md) — v6.4 增量记录（prompt-cache 排序 + 个人 KB 答案出处溯源；滚动合集，④⑤①② 待补）**
- **[v6.5-知识图谱链路治理](v6.5-知识图谱链路治理-20260628.md) — v6.5 增量记录（P0 静默失效治理 + 图谱接入召回 + 死代码清理 + per-fact trace；知识卡片链路对抗验证审计驱动）**
...
**v6.4 的本质升级（滚动合集，两条独立路线）**：(A) **prompt-cache 排序**——SystemMessage 段改为稳定段前置、时钟锚点置末，让跨轮稳定前缀可命中 DeepSeek prompt cache（顺序对是必要不充分，真实增益待 token 观测）；(B) **个人 KB 答案出处溯源**——把原本只注入模型上下文就被丢弃的 `RecallHit` 接通到前端：`SourceRef` 派生 4 类 Kind（记忆/文档/卡片/公开）、按类分组展示，让"每条答案有出处"（个人 KB 可信=可溯源的差异化体验）。

**v6.5 的本质升级**：把"知识图谱"做成名副其实——`CardGraphExpander` 在召回 expand 阶段沿 `card_relation` 取 1 跳邻居（precedes/derived_from/contains 加权 0.9 ＞ related 0.6）、以来源「图谱」注入，精心建模的关系类型从详情页装饰字段变成召回侧真正消费的图遍历；顺带治理全链路一类**"静默失效"反模式**（embedding 失败写 null 不再污染 dense_vector / 关联发现候选超预算降级补算 / 新增 ES↔MySQL 对账任务，与 memory `rerank-silent-failure-fix` 同源），并落 **per-fact trace**（`injected_facts`）让"召回注入了什么"可观测、可评估。起手是一次 8 组件并行深读 + 逐条对抗验证的审计（40 条为真发现）。
...
（完整架构图见 [v5 全景 · 二、全链路架构](../v5/v5-全景文档-20260608.md)；v6.1 Trace/Eval、v6.2 工具结果治理、v6.3 长上下文自缩放、v6.4 答案出处溯源/prompt-cache 排序、v6.5 知识图谱召回/静默失效治理/per-fact trace 是横切观测/治理/交付层，见 §7.3、§7.4、§7.5、§7.6、§7.7。）
...
## 四、后端源码目录（v6.0 / v6.1 / v6.2 / v6.3 / v6.4 / v6.5 新增标记 🆕）
...
chat/dto/                                         [v6.4]
  🆕 SourceRef.java              答案出处引用（RecallHit→前端；Kind 派生 + 文档名/去日期/去重）
  ✏️ ChatService.assembleSystemBlock  prompt-cache 排序：稳定段前置、时钟锚点置末
rag/pipeline/recall/ + card/service/ + rag/tracing/   [v6.5]
  🆕 CardGraphExpander.java       知识图谱邻居扩展器（命中卡片沿 card_relation 取 1 跳、按关系类型加权、来源「图谱」注入）
  🆕 KnowledgeCardEsReconciliationService.java  ES↔MySQL 对账（@Scheduled 回填缺 embedding/缺失 + 清孤儿；diffReconciliation 纯函数）
  ✏️ KnowledgeCardEsSyncService   embedding 失败写 null（不再污染 dense_vector）+ 对账查询 helper
  ✏️ CardRelationDiscoveryService  selectEmbeddingScope 候选超预算降级补算 + 删 keyword_relation/parent_id 死分支
  ✏️ RagTraceDocument / RagQualityLogger / RagRecall  per-fact trace（injected_facts + toInjectedFacts + 落 trace）
```

（v5 完整目录见 [v5 全景 · 四](../v5/v5-全景文档-20260608.md)。）

## 五、新增资源与配置 [v6.0 / v6.1 / v6.2 / v6.3 / v6.4 / v6.5]
...
fish.card.es-reconcile:       { enabled: true }   # [v6.5] 知识卡片 ES↔MySQL 对账任务开关（matchIfMissing=true，@Scheduled fixedDelay=120s）
```
...
## 六、新增测试文件 [v6.0 / v6.1 / v6.2 / v6.3 / v6.4 / v6.5]
...
- **[v6.4]** `SourceRefTest`（8：Kind 派生 MEMORY/DOC/CARD/PUBLIC + 文档名优先 + 去日期 + 去重/封顶）、`ChatServiceSystemBlockAssemblyTest`（3：prompt-cache 段顺序不变量）
- **[v6.5]** `KnowledgeCardEsSyncServiceTest`（6：embedding 失败写 null + 成功写向量 + 对账查询 helper 提取）、`CardRelationDiscoveryServiceTest`（2：selectEmbeddingScope 上限内全返 / 超限按 partial 分取 top）、`KnowledgeCardEsReconciliationServiceTest`（2：diffReconciliation 差分 + reconcile 回填/清孤儿接线）、`CardGraphExpanderTest`（4：1 跳邻居加权 / 去重 / 截断 / 无种子直通）、`RagQualityLoggerTest`（2：toInjectedFacts 映射 + 空值）
...
### 7.6 答案出处溯源 + prompt-cache 排序 [v6.4]
- **答案出处溯源**（个人 KB · Top1）：`Augmentation.buildAugmentationWithSources` 暴露 finalHits（`AugmentationResult(block, hits)`，向后兼容）→ `BuildMessagesResult.sources` → `onComplete` 在 `done` 前发 SSE `sources` → 前端 `MessageBubble` 按 `SourceRef.Kind`（MEMORY/DOC/CARD/PUBLIC，在 `from()` 从 sourceLabel + id `"card:"` 前缀派生）分组（笔记/卡片/公开/记忆）、chip 只显名称、snippet 进 hover tooltip、每组≤2+溢出。出处数据（RecallHit）端到端本就存在，v6.4 只是把"被丢弃的最后一段"接通到前端。
- **prompt-cache 排序**（长上下文 A 线）：`ChatService.assembleSystemBlock` 把 SystemMessage 段改为稳定段（instruction/摘要/摘录）前置、易变段（RAG/会话状态/秒级时钟锚点）后置，让跨轮稳定前缀可命中 DeepSeek prompt cache（顺序对是必要不充分，真实命中增益待 token 观测）。

### 7.7 知识图谱召回 + 静默失效治理 + per-fact trace [v6.5]
- **图谱接入召回**：精排后 expand 阶段 `CardGraphExpander.expand(contextExpander.expand(provenance))`——对命中卡片沿 `card_relation` 取 1 跳邻居，按关系类型加权（precedes/derived_from/contains=0.9 ＞ related_to=0.6），与已命中去重、按「种子分×权重」截断上限 4，以来源「图谱」注入（**绕过精排**，相关性来自图关系而非查询匹配）。`precedes`/`derived_from`/`contains`/`related` 真正进上下文。
- **静默失效治理**（A 线，与 memory `rerank-silent-failure-fix` 同源）：① embedding 失败写 `null`（Spring Data 省略 dense_vector 字段，文档仍落 ES、kNN 自动跳过，不再整篇被 ES 拒收）；② 关联发现候选对超 100 时 `selectEmbeddingScope` 按其余三信号降序取 top-100 降级补算 + `log.warn`（不再整体置零）；③ `KnowledgeCardEsReconciliationService`（`@Scheduled fixedDelay=120s`）回填缺 embedding / 缺失的 confirmed 文档 + 清孤儿，`diffReconciliation` 纯函数做集合差分。
- **per-fact trace**（C 线）：`RagTraceDocument.injected_facts`（object：id / source_label / score）落每轮注入事实明细，`RagQualityLogger.toInjectedFacts` 映射 `finalHits`；图谱邻居以 `source_label=图谱` 出现，可观测、可评估。

## 八、数据模型 [v6.0 / v6.1 / v6.2 / v6.5]
...
| `fish-trace`（TurnTrace）[v6.1] | `nodes[]{...,disposition}`（v6.2 工具结果治理处置方式）、`rag_injected/memory_injected`、`token_usage`、`status`、**`injected_facts[]{id,source_label,score}`[v6.5]**（每轮注入事实 per-fact 明细，图谱邻居 source_label=图谱） |
...
### v6.4 新增面试话题
- **个人 KB 答案为什么要有出处**：每条事实都"有主"（你的笔记/卡片/记忆），可信=可溯源是相对通用搜索的差异化；RecallHit 数据端到端本就有，只是被丢弃，v6.4 把它接通到前端。
- **出处怎么分类分组**：`SourceRef.Kind`（MEMORY/DOC/CARD/PUBLIC）在 `from()` 从现有字段派生（sourceLabel + id `"card:"` 前缀），**不动 RecallHit**——对抗验证后选的最小改动面（避免第 11 参 + 4 检索器改动）。
- **出处如何不喧宾夺主**：分组小标题 + 每组≤2 + chip 只显名称（snippet 进 tooltip）+ 记忆组最弱化；答案才是主体。
- **prompt-cache 为什么要把时钟行挪到末尾**：DeepSeek 严格前缀匹配，秒级时间戳在首 token 会让整段 system 前缀每轮全量 miss；挪到末尾让稳定前缀可命中（真实增益待 token 观测证实）。

### v6.5 新增面试话题
- **"知识图谱"和普通向量库的区别（曾被问倒）**：之前 `card_relation` 建模了 precedes/derived_from/contains/related 却从不参与召回——召回只做文本+向量检索，图遍历能力为零，关系类型是装饰字段。`CardGraphExpander` 让命中卡片沿边取 1 跳邻居注入，关系才真正进上下文；起手是一次 8 组件并行深读 + 逐条对抗验证的审计（40 条为真发现）。
- **为什么图邻居绕过精排**：它的相关性来自与种子的图关系（precedes/related），而非与查询文本的匹配；与 `ContextExpander` 加 chunk 邻块同理——按结构邻接补上下文，不参与查询相关性排序。
- **一类"静默失效"反模式（与 rerank 同源）**：embedding 失败写空数组污染 dense_vector 致整篇文档被 ES 拒收、关联发现候选超 100 整体丢 embedding 信号、ES 写删失败仅 warn 无对账——统一用"降级且留痕 + 定时对账"治，不要"静默且丢弃"。
- **ES↔MySQL 双写最终一致性怎么兜**：对账任务做差集（backfill 缺 embedding/缺失 + orphan 清理），能修任何来源的不一致（网络抖动/ES 重启/吞异常）；比内存重试/outbox 更简单，代价是 ~120s 一致性窗口。
- **per-fact trace 解决什么**：聚合指标（recall_total_hits/injected_fact_count）回答不了"注入了哪些、来自哪、图谱邻居有没有用"；`injected_facts` 让召回可回归验证，是 v6.1 eval 在图谱子系统的地基。
...
| **v6.4** | **2026-06-27** | **滚动更新（双线）：(A) prompt-cache 排序——SystemMessage 稳定段前置/时钟锚点置末；(B) 个人 KB 答案出处溯源——RecallHit→SourceRef(Kind 派生)→SSE sources→前端按类分组。SourceRefTest 8 + ChatServiceSystemBlockAssemblyTest 3，全量 260/260 绿、vue-tsc 干净。④⑤①② 待补** |
| **v6.5** | **2026-06-28** | **知识图谱链路治理合集（审计驱动，8 组件对抗验证 40 条为真发现）：(A) P0 静默失效——embedding 失败写 null 不再污染 dense_vector + 关联发现候选超预算降级补算 + ES↔MySQL 对账任务；(B) 图谱接入召回——`CardGraphExpander` 沿 `card_relation` 取 1 跳邻居注入 + 删 `keyword_relation`/`parent_id` 死分支（表/实体保留）；(C) per-fact trace（`injected_facts`）。16 个新测试，全量 276/276 绿（除需真实 infra 的 contextLoads）。未提交** |

（v1~v5.6 完整版本历史见 [v5 全景 · 十](../v5/v5-全景文档-20260608.md)。）

---

_2026-06-28 更新 · v6.0 ~ v6.5 全景_

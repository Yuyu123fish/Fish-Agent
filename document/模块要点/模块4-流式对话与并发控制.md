...
基于 **SseEmitter** 的 POST SSE 流式对话，前端 **Fetch + ReadableStream** 逐字渲染 Markdown；通过 **Redis Lua 令牌桶 + SSE 并发槽 + 会话互斥锁** 三层防护，以 **429/409** 明确区分频率与并发拒绝，回调注册顺序防竞态锁泄漏。**MDC traceId 全链路传播 [v5.0]：TraceFilter 入口注入 → MdcAsync 异步自动传播 → Reactor/SSE 回调手动恢复。** **Token 预算 [v5.4]：按模型窗口分配 inputBudget，P0 输入 + P1 系统指令先留，剩余按权重切给 window/RAG/摘要/摘录/状态，emergencyTrim 二次兜底。** **[v6.3] 模型窗口改由 `ActiveChatModelContext` 单真相源提供（命中 `model-context-overrides` 否则回退默认），与工具结果治理共用同一份"窗口有多大"。** **ChatTurnLifecycle CAS 收尾 [v6.1]：success/error/timeout 四路一次性回调，落 trace + 清 scratch [v6.2]。**
...
### 4. Token 预算与上下文裁剪（v5.4）

`[ContextBudgetAllocator.java](../../src/main/java/com/yuyu/fishagent/chat/budget/ContextBudgetAllocator.java)`

每轮 `buildMessages` 先按**当前模型的上下文窗口**算出 inputBudget，再按优先级把窗口分给各段，保证拼出来的 prompt 不超模型窗口：

```java
// inputBudget = max(1000, contextWindow × (1 - safetyRatio) − outputReserve)
// [v6.3] 窗口来自 ActiveChatModelContext 单真相源（命中 model-context-overrides 否则回退默认），
//        与工具结果治理（模块 1）共用同一份"窗口有多大"，迁移不再漏改 override
int contextWindow = activeChatModelContext.effectiveContextWindow();
BudgetPlan plan = new ContextBudgetAllocator(contextWindow, outputReserve, safetyRatio)
...
| P0 | 当前用户输入 | 先预留（不计权重） |
| P1 | 系统必需（时钟 + 人设） | 先预留 |
| — | window（滑动窗口历史） | remaining × **40%** |
| — | RAG（长期记忆/知识库） | remaining × **25%** |
| — | summary（结构化短期摘要） | remaining × **20%** |
| — | excerpt（关键原文片段） | remaining × **10%** |
| — | state（Agent 过程状态） | 剩余 ~5% |

每段注入前用 `TokenEstimator.estimate` 检查自身预算，超了就截断并标 `trimmed`（如 `trimTextToBudget` 二分按字符切到 token 预算内）。

**二次兜底 `emergencyTrim`**：各段都切完后若总量仍超 inputBudget（段间分隔符开销等极端情况），降级为只保留 **P1 系统必需 + P0 当前输入 + 尽量多的近期窗口**，其余（摘要/RAG/摘录/状态）全丢。这是"宁可丢增强上下文也要保证本轮能跑"的最后防线。

### 5. 对话生命周期 CAS 收尾（v6.1 / v6.2）

`[ChatTurnLifecycle.java](../../src/main/java/com/yuyu/fishagent/common/metrics/ChatTurnLifecycle.java)` + `ChatService.streamChat`

每轮对话用 `ChatTurnLifecycle.start(chatMetrics, finishCallback)` 包裹，finishCallback 用 **AtomicBoolean CAS 保证整轮只执行一次**，覆盖所有终止路径：

```java
final ChatTurnLifecycle turnLifecycle = ChatTurnLifecycle.start(chatMetrics, outcome -> {
    var completedTrace = traceCollector.finishTurn(turnId, outcome);   // 收尾 trace
    if (completedTrace != null) {
        traceEsWriter.persistAsync(completedTrace);                    // async 写 ES
        traceCollector.remove(turnId);
    }
    toolResultGovernor.clearScratch(turnId);                           // [v6.2] 清 scratch 两 key
});
```

**五路都会触发** `turnLifecycle.success()/error(...)`：① 正常 done（success）；② Reactor 流式异常（error）；③ SSE timeout；④ SSE completed-before-finish（客户端提前断）；⑤ 抢会话锁失败 / 组装失败（error）。CAS 保证哪怕多个终止信号并发到达，收尾逻辑（落 trace + 清 scratch）也只跑一次。

**Trace 透传**：`TraceContext.setTurnId(turnId)` 在订阅前设置，`chatAgent.stream(messages, sid, turnId)` 把 turnId 绑到工具集（v6.2 turn-bound）；`recordRagInjected/recordMemoryInjected` 记录本轮真实注入的增强片段，turn 结束一次性 async 写 ES（不进 Redis、不阻塞热路径）。
...
| **Token 预算分配 [v5.4]** | `chat/budget/ContextBudgetAllocator.java`、`BudgetPlan.java`、`ContextWindowTrimmer.java` |
| **模型窗口 / 预留配置 [v5.4]** | `llm/config/FishLlmProperties.java`（`model-context-overrides` / `output-reserve-tokens` / `safety-margin-ratio`） |
| **有效窗口单真相源 [v6.3]** | `llm/config/ActiveChatModelContext.java`（effectiveContextWindow，allocator/governor 共用） |
| **对话生命周期 CAS 收尾 [v6.1]** | `common/metrics/ChatTurnLifecycle.java`（一次性 finish 回调） |
| **TurnTrace 透传 + async 写 ES [v6.1]** | `common/trace/TraceCollector.java`、`TraceContext.java`、`TraceEsWriter.java` |
| **scratch 清理（CAS 回调内）[v6.2]** | `agent/tool/result/ToolResultGovernor.java`（clearScratch） |

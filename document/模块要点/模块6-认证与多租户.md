...
`[UserMemoryElasticsearchSearcher.java](src/main/java/com/yuyu/fishagent/agent/memory/rag/recall/UserMemoryElasticsearchSearcher.java)`：
...
| 认证服务 | `service/AuthService.java` |
| 认证控制器 | `controller/AuthController.java` |
| 认证配置 | `config/AuthProperties.java` |
| 用户实体 | `entity/SysUser.java` |
| 用户 Mapper | `mapper/SysUserMapper.java` |
| ES 私有检索（含 user_id filter） | `agent/memory/rag/recall/UserMemoryElasticsearchSearcher.java` |
| ES 知识检索（含 user_id filter） | `agent/memory/rag/recall/UserKnowledgeElasticsearchSearcher.java` |
| ES 公共检索（无 filter） | `agent/memory/rag/recall/PublicKnowledgeElasticsearchSearcher.java` |

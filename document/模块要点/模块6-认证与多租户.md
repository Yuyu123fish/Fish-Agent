...
基于 **自定义 token + Redis Session** 的无状态鉴权体系，通过 **`TraceFilter` 全链路 traceId 注入 [v5.0]** → **`GlobalAuthInterceptor` + `ThreadLocal<UserContext>`** 全链路携带用户身份，在 **ES 三索引**中以 `user_id` term filter 强制隔离私有数据，**虚拟线程**下显式快照回放防止 ThreadLocal 丢失。
...
flowchart TB
    ...
    subgraph Intercept["请求处理链"]
        TraceF["TraceFilter [v5.0]<br/>MDC.put(traceId)<br/>@Order(HIGHEST_PRECEDENCE)"]
        Header["X-Auth-Token"]
...
拦截器链顺序（v5.0 更新）：
```
TraceFilter（Filter 层，@Order(HIGHEST_PRECEDENCE)）[v5.0]  ← 最早，MDC traceId 注入
  → GlobalAuthInterceptor（/api/**）                         ← userId → UserContextHolder
  → RateLimitInterceptor（/api/chat/**）                     ← 需 userId，必须排在 Auth 后
  → PermissionInterceptor（/api/**）                         ← 角色权限
```

注意：`TraceFilter` 是 `OncePerRequestFilter`（Servlet Filter），早于所有 Spring MVC Interceptor 执行。它的 `@Order(HIGHEST_PRECEDENCE)` 保证最先注入 MDC traceId，后续所有 Interceptor 和 Controller 中的日志自动携带 traceId。`finally` 块中 `MDC.remove(traceId)` 清理，防止线程复用时 traceId 泄漏。
...
> **v6.0 补充**：`fish-user-memory` 召回双腿（BM25 + kNN）额外加 `must_not superseded=true`——被 supersede 的失效事实（北京→上海的旧"北京"）不召回，模型只见 active 事实（兼容旧数据无该字段）。记忆/知识 searcher 还读 `doc.authority`（PUBLIC=1.0/PRIVATE=0.7）供 `ProvenanceBooster` 加权（见模块 7）。
...
**v5.0 补充 [v5.0]**：同样的问题也存在于 `MDC` 的 traceId——虚拟线程切换 carrier thread 时，SLF4J MDC 也会丢失。v5.0 引入 `MdcAsync` 工具类替代裸 `CompletableFuture`，自动在异步任务入口恢复 MDC、出口清理 MDC。项目 11 处异步任务已全部迁移：6 处 `MdcAsync.mdcSupplyAsync/mdcRunAsync`（RagRecall/RagQueryExpand/RagHydeService/RagQualityLogger），5 处 Reactor/SSE 回调通过 `MDC.setContextMap(streamMdcSnapshot)` 手动恢复。两个 ThreadLocal（`UserContextHolder` + `MDC`）在虚拟线程下采用相同的显式快照-回放-清理策略。
...
## 最难/最有挑战的事情：虚拟线程下 ThreadLocal 的"隐形丢失"

### 问题
项目使用 JDK 21 虚拟线程 + `ThreadLocal<UserContext>` 传递用户身份。`GlobalAuthInterceptor` 在 Servlet 线程写入 `UserContextHolder.set(ctx)`，后续 Service/DAO 通过 `UserContextHolder.get()` 获取 userId。

### 挑战
一切在 Servlet 线程上正常。但 RAG 检索和记忆压缩用 `CompletableFuture.runAsync()` 异步执行——在虚拟线程的 ForkJoinPool 中，carrier thread 可能随时切换，**ThreadLocal 不会自动传递到新 carrier**。现象：RAG 检索日志中 `user_id` 始终为 null，私有知识检索返回空列表。这个 bug 不是 100% 复现——只有在 carrier thread 切换时才出现，调试时碰巧没切换就通过，上线后随机触发。
...
v5.0 将同一模式应用于 MDC traceId，并封装 `MdcAsync` 工具类自动传播 MDC。

为什么不选 `InheritableThreadLocal`：线程池场景下子线程是复用的，第二次提交时可能带着上一次的脏值。为什么不选 `ScopedValue`（Java 21 Preview）：Spring Boot 3.5 未适配。

### 面试回答要点
> "虚拟线程下 ThreadLocal 丢失是 JDK 21 的已知行为——carrier thread 切换时 ThreadLocal 不保证传递。最坑的是这个 bug 不是必现的——取决于 ForkJoinPool 的调度。解决方案是显式快照-回放-清理三步：入口把 UserContext 存到局部变量，异步 lambda 内 set 回去，finally 中 clear。代码量只多三行，但行为完全可控。`InheritableThreadLocal` 在线程池场景有脏值风险，`ScopedValue` Spring 还没适配，显式传递是当前最稳的方案。"

...
基于 **LLM 对话提取 + 人工确认** 的知识沉淀闭环，通过 **关键词实体归一化**、**多信号加权投票关联发现**、**树形分组层级** 和 **MySQL→ES 双写同步**，将对话中的散碎知识组织为可检索、可复习、可图谱化浏览的结构化卡片，并经 **四路 RAG** 回注对话上下文。
...
### 6. KnowledgeCardService — CRUD 与事务安全
...
```
手动创建 → CONFIRMED (直接可用)
AI 提取 → PENDING → CONFIRMED (ES 双写) / REJECTED (ES 清除)
编辑 confirmed 卡片 → MySQL 更新 + ES 更新
合并 → keep 保留 + discard 关键词合并 + 关联迁移 + discard 删除
删除 → 子表清空 → 关联拆分删除 → 主表删除 → ES 删除
```

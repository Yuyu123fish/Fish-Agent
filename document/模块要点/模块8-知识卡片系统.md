...
## 最难/最有挑战的事情：Spring Cache @Cacheable 命中时绕过权限校验的越权漏洞

### 问题
v5.0 为知识卡片引入 `@Cacheable` 缓存。`KnowledgeCardService.detail(id)` 方法内部包含 `findOwnedCardOrForbidden(userId, id)` 权限校验——用户只能看自己的卡片。

### 挑战
`@Cacheable` 的工作机制是：**缓存命中时直接返回缓存值，跳过方法体执行**。这意味着 `detail()` 的权限校验在缓存命中时被完全绕过。如果缓存 key 只用 `cardId`（`key="#id"`），用户 A 查卡片 123 缓存后，用户 B 查同一张卡片会命中缓存直接返回——**权限绕过漏洞**。

### 解决方案
将 userId 编入缓存 key：

```java
@Cacheable(value = CARD_DETAIL, key = "T(com.yuyu.fishagent.auth.context.UserContextHolder).currentUserIdOrNull() + ':' + #id")
public KnowledgeCardVO detail(Long id) {
    return findOwnedCardOrForbidden(UserContextHolder.currentUserIdOrNull(), id);
    // 权限校验在 cache miss 时执行
}
```

- 用户 A 查卡片 123 → cache key = `1:123` → miss → 权限校验 → 写缓存
- 用户 B 查卡片 123 → cache key = `2:123` → miss（不同 key）→ 权限校验 → 403

额外保障：`transactionAware()` 让 `@CacheEvict` 延迟到事务提交后执行，避免"驱逐缓存 → 事务回滚 → 缓存被误清"。

### 面试回答要点
> "Spring Cache @Cacheable 有个隐含行为——缓存命中时跳过方法体。如果方法体包含权限校验，那缓存命中等于越权。解决方案是把 userId 编入缓存 key，让不同用户的缓存互不干扰。这看似简单，但很多项目在加缓存时没意识到这个问题。事务感知驱逐也很重要——如果驱逐在事务提交前执行，回滚后缓存已空，下次读到旧数据。"

---

## v6.5 增量：知识卡片链路治理（STAR，审计驱动）

> 起手是一次 8 组件并行深读 + 逐条对抗验证的审计（40 条为真发现、5 条伪阳性剔除）。本节只列影响**知识卡片模块**的四项；完整记录见 [v6.5 文档](../v6/v6.5-知识图谱链路治理-20260628.md)。

### A. embedding 失败不再写空数组（静默失效）

- **Situation**：`KnowledgeCardEsSyncService.embedForCardQuietly` 失败返回 `List.of()` → 空数组写进 `dense_vector` → ES 维度校验**拒收整篇文档** → 被 catch 吞掉 → 卡片在 ES 彻底不存在（全文+向量都召回不到），方法注释却写"保证全文能力"。
- **Task**：失败要"降级且留痕"，不要"静默且丢弃"。
- **Action**：两条失败路径返回 `null`（不再 `List.of()`）；Spring Data `MappingElasticsearchConverter` 默认省略 null 字段 → 文档正常落 ES、只是没有 embedding 字段 → 全文照常、kNN 自动跳过。`findSimilarConfirmed` 加 null 守卫。
- **Result**：从"整篇丢失"变"优雅降级（仅向量路跳过）"。`KnowledgeCardEsSyncServiceTest` 覆盖三态（抛异常 / 不可用 / 成功）。

### B. 关联发现候选超预算降级补算（不再整体置零）

- **Situation**：`CardRelationDiscoveryService` 候选对 `>100` 时 embedding 评分直接 `Map.of()` 整体置零 → 越是大库越需要语义区分的场景 S4 信号反而被关掉，且无日志。原 `candidates.size()` 开关偏保守（成本随**去重 anchor 数**走，而非候选对数）。
- **Task**：头部候选保留 embedding 信号、长尾才丢；绝不静默。
- **Action**：抽纯函数 `selectEmbeddingScope`——候选对 `>100` 时按其余三信号（关键词 Jaccard / 同组 / 关键词层次）降序取 top-100 对再算 embedding，并 `log.warn` 降级提示。
- **Result**：`CardRelationDiscoveryServiceTest` 覆盖（上限内全返 / 超限按 partial 分取 top）。

### C. ES↔MySQL 对账任务（最终一致性兜底）

- **Situation**：ES 写/删失败仅 `log.warn` 吞掉 → MySQL 已 confirmed、ES 缺文档，**无重试 / 对账 / 补偿**，卡片永久消失且用户无感（前端列表走 MySQL 仍看得到，掩盖了问题）。
- **Task**：给双写最终一致性兜底，能修任何来源的不一致。
- **Action**：新增 `KnowledgeCardEsReconciliationService`（`@Scheduled fixedDelay=120_000`，镜像 `OrphanTaskCompensationService`）——回填缺 embedding / 缺失的 confirmed 文档 + 清孤儿；纯函数 `diffReconciliation` 做集合差分（`backfill = ES 缺 embedding ∪ (MySQL 有 ES 无)`、`orphan = ES 有 MySQL 无`）；ES 查询辅助放 `KnowledgeCardEsSyncService`。`fish.card.es-reconcile.enabled`（`matchIfMissing=true`）可关。
- **Result**：`KnowledgeCardEsReconciliationServiceTest` 覆盖（差分 + 回填/清孤儿接线）。本轮不做 afterCommit 解耦（对账已覆盖两方向）。

### D. 删 keyword_relation / parent_id 死分支

- **Situation**：`keyword_relation` 表 + `keyword.parent_id` 注释了 synonym/broader/narrower 语义，但**全工程零写入路径**——关联发现的 S3（关键词层次/关系）扩展恒为空集，是"建了不写"的灰色地带。
- **Task**：让代码不再撒谎；表/实体/mapper 类保留供将来补写入路径。
- **Action**：`CardRelationDiscoveryService.buildExpandedKeywordMap` 里读 `keyword_relation` / `parent_id` 的恒空分支删掉，移除 `keywordRelationMapper` 字段；S3 信号诚实为 0。
- **Result**：行为不变（S3 本就恒 0），代码不再读恒空结构误导后人。
...
| **ES↔MySQL 对账任务 [v6.5]** | `card/service/KnowledgeCardEsReconciliationService.java` |
| **embedding 失败治理 / 对账查询 [v6.5]** | `card/service/KnowledgeCardEsSyncService.java`（`embedForCardQuietly` 返 null + `listConfirmed*CardIds`） |
| **候选超预算降级 / 删死分支 [v6.5]** | `card/service/CardRelationDiscoveryService.java`（`selectEmbeddingScope` + 精简 `buildExpandedKeywordMap`） |

...
**Java 负责投递 + Python Worker 异步消费**，通过 **Redis Stream 消费者组**解耦，实现 **6 种格式解析 → OCR 清洗 → 结构感知分块 → embedding → ES 写入**全链路；配合 **Worker 心跳**、**XAUTOCLAIM 崩溃恢复**、**Java 定时孤儿补偿**、**Python 幂等写入**与 **embedding 指数退避重试**五重保障。**v6.0 索引期上下文前缀（contextual retrieval）**：分块后、embed 前给每块前缀文档上下文（content/context_prefix/contextualized_content 三字段，BM25+embedding 走 contextualized），解决孤立小块丢上下文；并注入 authority（PUBLIC=1.0/PRIVATE=0.7）。
...
### 10. 索引期上下文前缀（v6.0 contextual retrieval，攻 Q7）

`[contextual_indexing.py](../../python/fish_worker/chunker/contextual_indexing.py)`

大文档拆片后，孤立小块召回精度高但**丢上下文**——一个讲"部署步骤"的 chunk 脱离文档标题后语义模糊。v6.0 在 chunker 出片后、embed 前给每块**前缀文档上下文**（Anthropic Contextual Retrieval 思路）：

```python
def apply_contextual_prefixes(chunks, document_text, *, enabled, generator=None, fallback_title=""):
    for chunk in chunks:
        prefix = generator(document_text, chunk) if generator else ""
        if not prefix:
            prefix = _fallback_prefix(...)        # 确定性启发式：标题 + 第N页 + chunk序号
        chunk.context_prefix = prefix
        chunk.contextualized_text = f"{prefix}\n\n{chunk.text}"   # BM25 + embedding 走它
    # content 字段保持原文，渲染/审计仍展示原始 chunk
```

- **三字段写 ES**：`content`（原文，渲染用）/ `context_prefix` / `contextualized_content`（=前缀+content，**BM25 + embedding 都走它**）。Java BM25 改 `contextualized_content + content` 双字段 should 查询，**兼容旧索引**。
- **可插拔 generator**：当前是确定性启发式前缀（标题+页+序号），未接在线 LLM，避免摄取链路引入外部依赖；生产 generator = DeepSeek + prompt 缓存（文档作稳定前缀吃满自动 context caching 省 ~90%），接口已预留，后续同一函数注入即可。
- **authority 注入**：`config.py` `fish_rag_authority_public=1.0` / `private=0.7`，入库写 `authority` 字段，供 Java 读侧 `ProvenanceBooster` 加权（见模块 7）。
- **诚实边界**：embedding 改用 contextualized 文本，**旧文档需全量重索引才受益**；首轮审查发现此前无 1.0 来源（"官方"是死代码），v6.0 打通。
...
## 最难/最有挑战的事情：Worker 心跳 vs 孤儿补偿的"假死"误判

### 问题
Python Worker 处理文档时（特别是 PDF OCR），大文件可能耗时 5-8 分钟。Java 侧的 `OrphanTaskCompensationService` 每分钟扫描 `status=PROCESSING AND updated_at < NOW() - 10min` 的任务。

### 挑战
v3.2 之前，Worker 开始处理时只在 `status='PROCESSING'` 时写一次 `updated_at`。如果 OCR 处理耗时 8 分钟，补偿服务在第 10 分钟认为 Worker 已死，执行 ES 清理 + MySQL 标记 FAILED——但 Worker 还在活着！这就是"假死误判"：补偿服务把还在工作的 Worker 当成了崩溃。

### 解决方案
v3.2 引入 Worker 心跳机制：
- `processor.py` 每 30 秒执行 `UPDATE updated_at=NOW() WHERE task_id=? AND status='PROCESSING'`
- 孤儿补偿判断条件不变（`updated_at < NOW() - 10min`），但只要 Worker 还在处理，`updated_at` 就会持续刷新
- CAS 更新：Worker 完成时 `UPDATE status='SUCCESS' WHERE task_id=? AND status='PROCESSING'`——如果补偿已标记 FAILED，CAS 不匹配，Worker 不会覆盖

```python
# processor.py 中心跳线程
def _heartbeat_loop(self, task_id, stop_event):
    while not stop_event.is_set():
        mysql.refresh_heartbeat(task_id)  # UPDATE updated_at=NOW()
        stop_event.wait(30)  # 30 秒间隔
```

### 面试回答要点
> "假死误判是分布式系统中经典的'心跳 vs 超时'问题。解决思路很直觉：Worker 定期'报平安'，补偿方只杀'10 分钟没报平安的'。但实现上有细节——CAS 更新保证补偿和 Worker 不会同时操作同一行：如果补偿先标记 FAILED，Worker 的 SUCCESS CAS 不匹配就静默退出，不会覆盖补偿结果。"
...
| **索引期上下文前缀 [v6.0]** | `python/fish_worker/chunker/contextual_indexing.py`（apply_contextual_prefixes，可插拔 generator） |
| TextChunk 字段（+context_prefix/contextualized）[v6.0] | `python/fish_worker/chunker/text_chunker.py` |
| 句子分割器 | `python/fish_worker/chunker/sentence_splitter.py` |
| authority 配置（PUBLIC=1.0/PRIVATE=0.7）[v6.0] | `python/fish_worker/config.py`（`fish_rag_authority_*` / `fish_rag_contextual_indexing_enabled`） |

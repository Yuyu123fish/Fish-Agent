...
**双写降级**：`syncConfirmedQuietly()` 全 try-catch——**[v6.5] embedding 失败写 `null`（不再写空数组污染 dense_vector 致整篇被 ES 拒收）**，文档仍以文本字段写入、全文检索不受影响；ES 写失败仅 WARN 不回滚 MySQL，并由 [v6.5] 对账任务兜底（见 §四）。不用 CDC（引组件）/定时全量（有延迟），同步双写毫秒级且 MySQL 优先。
...
**3. ES 双写失败怎么办？**
syncConfirmedQuietly 全 try-catch，[v6.5] embedding 失败写 `null`（文档仍落 ES、kNN 自动跳过，不再整篇丢失），ES 写失败仅 WARN 不回滚 MySQL——用户创建卡片不应因 ES 暂时不可用收到错误；偶发不一致由 [v6.5] 对账任务（`@Scheduled 120s`）回填/清孤儿兜底（见 §四）。

---

## 四、v6.5：知识卡片链路静默失效治理（STAR，审计驱动）

> 起手是一次 8 组件并行深读 + 逐条对抗验证的审计（40 条为真发现）。本节列影响知识卡片的三项；完整见 [v6.5 文档](../../v6/v6.5-知识图谱链路治理-20260628.md)。**图谱接入召回**（CardGraphExpander）属于 RAG 侧，见模块 7。

**① embedding 失败不再写空数组**
- **Situation**：`embedForCardQuietly` 失败返回 `List.of()` → 空数组写进 `dense_vector` → ES 维度校验**拒收整篇文档** → 被 catch 吞掉 → 卡片在 ES 彻底消失（注释却写"保证全文能力"）。
- **Task**：失败要"降级且留痕"，不要"静默且丢弃"。
- **Action**：失败返 `null`（Spring Data 省略 null 字段）→ 文档正常落 ES、只是没 embedding → 全文照常、kNN 自动跳过。
- **Result**：从"整篇丢失"变"仅向量路跳过"。

**② ES↔MySQL 对账任务（最终一致性兜底）**
- **Situation**：ES 写/删失败仅 `log.warn` 吞掉，MySQL 已 confirmed、ES 缺文档，无重试/对账，卡片永久消失且用户无感。
- **Task**：给双写一致性兜底，修任何来源的不一致。
- **Action**：`KnowledgeCardEsReconciliationService`（`@Scheduled 120s`）回填缺 embedding/缺失文档 + 清孤儿，纯函数 `diffReconciliation` 做集合差分。
- **Result**：~120s 窗口的最终一致性，比内存重试/outbox 简单。

**③ 关联发现候选超预算降级补算**
- **Situation**：候选对 >100 时 embedding 评分整体置零 → 大库反而丢 S4 信号、且无日志。
- **Task / Action**：`selectEmbeddingScope` 超限时按其余三信号（关键词/同组/层次）取 top-100 对再算 embedding + `log.warn`。
- **Result**：头部保留 S4，长尾才丢。

> 同步清理：`keyword_relation` / `parent_id` 零写入的死分支删掉（表/实体保留），S3 信号诚实为 0——代码不再撒谎。

### 追问

**1. 一类"静默失效"反模式怎么治（与 rerank 同源）？**
失败要降级且留痕（embedding 写 null、候选超限降级补算 + warn），加定时对账兜底一致性，不要"静默且丢弃"——这类 bug 测试全 ASCII/正常路径时隐形，对抗验证审计才揪出来。

**2. ES↔MySQL 双写最终一致性怎么兜？**
对账任务做差集（backfill 缺 embedding/缺失 + orphan 清理），能修网络抖动 / ES 重启 / 吞异常任何来源的不一致；比内存重试简单（同事务同抖动大概率同步失败），代价 ~120s 一致性窗口。
...
| **ES↔MySQL 对账 [v6.5]** | `card/service/KnowledgeCardEsReconciliationService.java` |

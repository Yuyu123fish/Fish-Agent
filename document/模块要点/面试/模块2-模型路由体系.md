...
- **活跃模型 + 有效窗口 单一真相源 [v6.3]**：`ActiveChatModelContext` 收口"当前用哪个模型 / 窗口多大"，对话预算与工具结果治理共用——根除 V3→V4 迁移漏改窗口 override（`deepseek-v4-flash` 顶着 65536 跑了很久）。
...
## 四、活跃模型 + 有效窗口：单一真相源 [v6.3]

### 背景
"当前用哪个模型 / 它的窗口多大"原本散在 `ChatService` 私有方法里——对话预算分配取一次、工具结果治理取一次，没有统一入口。

### 问题与方案
新增 `ActiveChatModelContext`（`llm/config/`）收口两件事：`activeModelName()`（按 provider 读对应 `spring.ai.{provider}.chat.options.model`）+ `effectiveContextWindow()`（命中 `model-context-overrides` 用 override，否则回退默认）。对话预算（模块 4）与工具结果治理（模块 1）**共用同一份窗口**。

**踩坑**：V3→V4 迁移改了模型名，却忘了同步 `model-context-overrides`——`deepseek-v4-flash` 顶着 V3 的 65536（64K）跑了很久，实际只用了原生 1M 的 1/16。根因正是"模型名→窗口"映射散落、两处各取一份。收口成单真相源后迁移只改一处，已修正为 `1048576`。

### 追问

**1. 为什么不直接在用到的各处各自解析？**
能跑，但"窗口多大"会漂移——这次 override 漏改就是散落结构的结果。单真相源让对话预算和工具治理永远看到同一个数，迁移只改一处。代价是一个近乎无状态的 bean（一次 Map 查找 + 一次 Environment 读取）。
...
| **活跃模型 + 有效窗口 单真相源 [v6.3]** | `llm/config/ActiveChatModelContext.java`（allocator/governor 共用） |

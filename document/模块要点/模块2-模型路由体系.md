...
通过 **EnvironmentPostProcessor** 早期注入 + **`@ConditionalOnProperty`** 条件装配 + **`@Primary`** 收敛，实现 DashScope / Ollama / DeepSeek 三路对话路由与嵌入路由完全解耦，**切换只需一个环境变量**，记忆链路可选独立模型。**`ActiveChatModelContext` 收口「当前模型名 + 有效上下文窗口」为单一真相源 [v6.3]**——对话预算与工具结果治理共用，模型迁移不再漏改窗口 override。
...
### 4. ActiveChatModelContext — 活跃模型 + 有效窗口 单一真相源 [v6.3]

`[ActiveChatModelContext.java](../../src/main/java/com/yuyu/fishagent/llm/config/ActiveChatModelContext.java)`：

收口两件原本散落的事：① **当前用哪个模型**（`activeModelName()`：按 `chat-provider` 读 `spring.ai.{provider}.chat.options.model`，逻辑从 `ChatService` 私有方法迁出）；② **它的窗口多大**（`effectiveContextWindow()`：命中 `model-context-overrides` 用 override，否则回退 `context-window-tokens`）。`ContextBudgetAllocator`（对话预算，模块 4）与 `ToolResultGovernor`（工具结果治理，模块 1）**共用同一份"窗口有多大"**——根除"模型迁移漏改窗口 override"：本次 `deepseek-v4-flash: 65536` 即 V3（deepseek-chat）的 64K 遗留数，迁到 1M 模型后已修正为 `1048576`。激活模型由配置决定、运行期不热切换，按调用即时解析（一次 Map 查找 + 一次 Environment 读取，开销可忽略）。
...
**Q：为什么单开一个 `ActiveChatModelContext` 来管「当前模型名 + 窗口」？[v6.3]**

这两件事原本散在 `ChatService` 私有方法里——对话预算分配器取一次、工具结果治理取一次，"当前用哪个模型 / 窗口多大"没有统一入口。后果就是 V3→V4 迁移时改了模型名却忘了同步 `model-context-overrides`（`deepseek-v4-flash` 顶着 V3 的 65536 跑了很久，实际只用了原生 1M 的 1/16）。收口成单一真相源后两处共用、迁移只改一处。顺带把绝对的工具结果阈值改成随窗口自缩放（见模块 1），小窗/未知窗退回绝对下限不回归。
...
| **活跃模型 + 有效窗口 单一真相源 [v6.3]** | `llm/config/ActiveChatModelContext.java`（allocator/governor 共用） |

...
function edgeStyle(type: RelationType) {
  ...
  if (type === 'derived_from') return { color: p.derived, dashes: [2, 4], arrow: true }
...
function legendStyle(type: RelationType) {
  const p = palette.value
  const style: Record<string, string> = {}
  if (type === 'contains') style.borderColor = p.contains
  else if (type === 'precedes') style.borderColor = p.precedes
  else if (type === 'derived_from') { style.borderColor = p.derived; style.borderTopStyle = 'dashed' }
  else { style.borderColor = p.related; style.borderTopStyle = 'dashed' }
  return style
}
  ...
  <section ref="graphRef" class="graph-panel">
    ...
    <footer class="legend">
      <span v-for="item in relationOptions" :key="item.value" class="legend-item">
        <i :class="`line ${item.value}`" :style="legendStyle(item.value)" />
